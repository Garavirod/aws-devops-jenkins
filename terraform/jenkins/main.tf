...
output "ssh_connection_string_for_ec2" {
  value = format("%s%s","ssh -i /Users/garavirod/.ssh/jenkins_bookyland ubuntu@", aws_instance.jenkins_ec2_instance_ip.public_ip)
}
...
resource "aws_instance" "jenkins_ec2_instance_ip" {
    ...
    key_name = "jenkins_bookyland" # private ssh key (file_name)
...
resource "aws_key_pair" "jenkins_ec2_instance_public_key" {
  key_name = "jenkins_bookyland" # public ssh key (file_name)
